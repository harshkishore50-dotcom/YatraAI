...
const state = { destination:null, weather:null, pois:[], map:null, markers:[], selectedInterests:new Set(['culture','food']), access:new Set(), lastPlan:null, aiConnected:false };
...
async function fetchPOIs(lat,lon){
  // GET avoids the browser preflight that can make POST requests fail on GitHub Pages.
  const query=`[out:json][timeout:25];(nwr["tourism"~"attraction|museum|gallery|viewpoint|zoo|theme_park|artwork|heritage"](around:8000,${lat},${lon});nwr["amenity"~"restaurant|cafe|marketplace"](around:5000,${lat},${lon}););out center tags;`;
  let lastErr;
  const endpoints=[...new Set([...(CFG.OVERPASS_ENDPOINTS||[]),
    'https://overpass-api.de/api/interpreter',
    'https://overpass.kumi.systems/api/interpreter',
  ...
  for(const endpoint of endpoints){
    try{
      const data=await fetchJson(`${endpoint}?data=${encodeURIComponent(query)}`,{method:'GET',headers:{'Accept':'application/json'}});
...
function makeItinerary(days, pace, walking, group){
  ...
  for(let d=0; d<days; d++){
    ...
    out.push({day:d+1,date:wd?.date || '',weather:wd,stops:slots.map((p,i)=>({time:times[i],name:poiName(p),type:poiType(p),latLng:poiLatLng(p),tags:p.tags||{},local:p._score?.local,reason:reasonFor(p,wd,i),crowdRisk:wd?.rain>=65?'Low outdoors / weather-sensitive':'Unknown crowd',source:'OpenStreetMap / community mapped'}))});
  }
  return out;
}
...
function apiState(plan){
  return {
    weather: !!plan.weather?.current,
    places: plan.poiCount > 0,
    map: typeof window.L !== 'undefined',
    ai: state.aiConnected
  };
}
function renderApiStatus(plan){
  const st=apiState(plan);
  const wrap=$('#apiStatus');
  if(!wrap) return;
  const item=(label,ok,detail)=>`<div class="api-status-item ${ok?'ok':'warn'}"><span class="api-status-dot"></span><div><strong>${label}</strong><small>${detail}</small></div></div>`;
  wrap.innerHTML=[
    item('Weather',st.weather,st.weather?'Open-Meteo connected':'Weather could not be verified'),
    item('Places',st.places,st.places?'OpenStreetMap / Overpass connected':'Mapped places unavailable'),
    item('Map',st.map,st.map?'Leaflet + OpenStreetMap tiles ready':'Map library unavailable'),
    item('AI layer',st.ai,st.ai?'Secure endpoint configured':'Deterministic planner active')
  ].join('');
}
...
function renderResults(plan){
  ...
  const quality=state.aiConnected ? 'AI + live + sourced' : plan.poiCount>=6?'Live + sourced':plan.poiCount>=2?'Live + partial':'Limited live data'; $('#planQuality').textContent=quality;
  $('#itinerary').innerHTML=plan.itinerary.map(day=>`<div class="day-block"><div class="day-header"><div><div class="eyebrow">DAY ${day.day}</div><div class="day-title">${day.date ? esc(formatDay(day.date)) : 'Date unavailable'}</div></div><div class="day-meta">${day.weather?`${iconFor(day.weather.code)} ${Math.round(day.weather.max)}° / ${Math.round(day.weather.min)}° · ${day.weather.rain}% rain`:''}</div></div>${day.stops.map(stop=>`<div class="stop"><div class="time">${esc(stop.time)}</div><div class="stop-main"><h4>${esc(stop.name)}</h4><p>${esc(stop.reason)}</p><div class="badge-row"><span class="badge green">${esc(stop.sourceType||sourceBadge(stop.tags))}</span>${stop.local?'<span class="badge green">Local-first</span>':''}${stop.uncertainty?`<span class="badge amber">${esc(stop.uncertainty)}</span>`:''}${stop.tags.opening_hours?'<span class="badge">Opening hours mapped</span>':'<span class="badge amber">Hours not verified</span>'}</div></div><div class="source-mini">${esc(stop.source||'Source unavailable')}</div></div>`).join('')}</div>`).join('') || `<div class="day-block"><strong>We couldn't build a complete itinerary from live place data.</strong><p>Please retry or use a broader destination query.</p></div>`;
  renderApiStatus(plan); renderMap(plan); renderSources(plan); renderLocal(plan);
}
...
function renderMap(plan){
  ...
  if(typeof window.L==='undefined'){
    mapEl.innerHTML=`<div class="map-fallback"><strong>Interactive map unavailable</strong><p>The mapping library could not be loaded. Your itinerary still works, and each place can be opened in OpenStreetMap.</p></div>`;
    return;
  }
  try{
    if(!state.map){
      state.map=L.map(mapEl,{scrollWheelZoom:false,zoomControl:true}).setView([plan.destination.latitude,plan.destination.longitude],12);
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap contributors',crossOrigin:true}).addTo(state.map);
    } else {
      state.map.setView([plan.destination.latitude,plan.destination.longitude],12);
      state.markers.forEach(m=>m.remove());
    }
    state.markers=[];
    const destinationMarker=L.marker([plan.destination.latitude,plan.destination.longitude]).addTo(state.map).bindPopup(`<strong>${esc(plan.destination.name)}</strong><br>Trip destination`);
    state.markers.push(destinationMarker);
    const all=plan.itinerary.flatMap(d=>d.stops).slice(0,18);
    const bounds=[[plan.destination.latitude,plan.destination.longitude]];
    all.forEach((stop)=>{
      const lat=stop.latLng?.[0],lon=stop.latLng?.[1];
      if(!Number.isFinite(lat)||!Number.isFinite(lon)) return;
      const m=L.marker([lat,lon]).addTo(state.map).bindPopup(`<strong>${esc(stop.name)}</strong><br>${esc(stop.source)}`);
      state.markers.push(m); bounds.push([lat,lon]);
    });
    if(bounds.length>1) state.map.fitBounds(bounds,{padding:[30,30],maxZoom:14});
    setTimeout(()=>state.map?.invalidateSize(),350);
  }catch(err){
    console.error('Map render error',err);
    mapEl.innerHTML=`<div class="map-fallback"><strong>Map could not be rendered.</strong><p>YatraAI kept the sourced itinerary available instead of failing the whole trip.</p></div>`;
  }
}
...
async function buildPlan(){
  ...
  try{
    ...
    let itinerary=makeItinerary(days,pace,walkingValue,group);
    if(!itinerary.length){
      itinerary=Array.from({length:days},(_,i)=>({
        day:i+1, date:dayWeather(i)?.date||'', weather:dayWeather(i),
        stops:[{time:i===0?'10:00 AM':'10:30 AM',name:`Explore ${geo.name}`,type:'destination',latLng:[geo.latitude,geo.longitude],tags:{},local:false,reason:'Nearby place data could not be verified, so YatraAI will not invent attractions. Start with the destination area and re-check live data later.',crowdRisk:'Unknown crowd',source:'Destination geocoding — Open-Meteo'}]
      }));
    }
    const plan={destination:geo,days,pace,group,languageLabel:language==='hi'?'हिन्दी':language==='bn'?'বাংলা':language==='ta'?'தமிழ்':language==='mr'?'मराठी':'English',weather,pois,itinerary,poiCount:pois.filter(p=>poiName(p)).length,ai:null}; state.lastPlan=plan;
    if(CFG.AI_ENDPOINT){
      setLoading(true,'YatraAI is reasoning…','Using the live evidence to build a source-aware itinerary.');
      try{ await enrichWithAI(plan, {language, budget:$('#budget').value, notes:$('#notes').value}); }catch(e){ state.aiConnected=false; console.warn('AI endpoint unavailable; using deterministic planner.',e); toast('AI is unavailable; showing the verified fallback plan.'); }
    }
    renderResults(plan);
  }catch(e){console.error(e);toast(e.message||'Something went wrong.');}
  finally{setLoading(false);}
}
async function enrichWithAI(plan, meta={}){
  const places=pickPOIs().slice(0,24).map((p,idx)=>({
    id:`p${idx+1}`, name:poiName(p), type:poiType(p), latLng:poiLatLng(p), local:!!p._score?.local,
    tags:{opening_hours:p.tags?.opening_hours||null, website:p.tags?.website||null, brand:p.tags?.brand||null}
  })).filter(p=>Array.isArray(p.latLng)&&p.latLng.every(Number.isFinite));
  const payload={
    destination:{name:plan.destination.name, latitude:plan.destination.latitude, longitude:plan.destination.longitude},
    trip:{days:plan.days, pace:plan.pace, group:plan.group, language:meta.language||'en', budget:meta.budget||'mid', walking:walkingValue, preferences:[...state.selectedInterests], accessibility:[...state.access], notes:meta.notes||''},
    live:{weather:plan.weather||null},
    mappedPlaces:places,
    rules:{neverInventFacts:true, onlyUseMappedPlaceIds:true, openingHoursMustComeFromTags:true, uncertaintyRequired:true, preferLocal:true}
  };
  const data=await fetchJson(CFG.AI_ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
  if(!data?.itinerary?.length) throw new Error('AI returned no usable itinerary.');
  const byId=new Map(places.map(p=>[p.id,p]));
  const normalized=data.itinerary.map((day,i)=>({
    day:day.day||i+1, date:day.date||dayWeather(i)?.date||'', weather:dayWeather(i),
    stops:(day.stops||[]).map(stop=>{ const src=byId.get(stop.placeId); if(!src) return null; return {time:stop.time||'Flexible',name:src.name,type:src.type,latLng:src.latLng,tags:src.tags,local:src.local,reason:stop.reason||'Selected using your preferences and the supplied live evidence.',source:stop.source||'Mapped place — OpenStreetMap',sourceType:stop.sourceType||'AI recommendation · evidence-linked',uncertainty:stop.uncertainty||'AI-generated recommendation'}; }).filter(Boolean)
  })).filter(d=>d.stops.length);
  if(!normalized.length) throw new Error('AI returned only unsupported places.');
  plan.itinerary=normalized; plan.ai=data.meta||{provider:'AI',confidence:'evidence-linked'}; state.aiConnected=true;
  toast('YatraAI AI engine connected.');
}
async function refreshPlan(){ if(!state.lastPlan){toast('Plan a trip first.'); return;} $('#results').scrollIntoView({behavior:'smooth'}); setLoading(true,'Re-checking your plan…','Fetching fresh weather and mapped places.'); try{ state.weather=await fetchWeather(state.destination.latitude,state.destination.longitude); state.pois=await fetchPOIs(state.destination.latitude,state.destination.longitude); state.lastPlan.weather=state.weather; state.lastPlan.pois=state.pois; state.lastPlan.itinerary=makeItinerary(state.lastPlan.days,state.lastPlan.pace,walkingValue,state.lastPlan.group);
    if(!state.lastPlan.itinerary.length){
      state.lastPlan.itinerary=[{day:1,date:dayWeather(0)?.date||'',weather:dayWeather(0),stops:[{time:'10:00 AM',name:`Explore ${state.destination.name}`,type:'destination',latLng:[state.destination.latitude,state.destination.longitude],tags:{},local:false,reason:'Nearby place data could not be verified. YatraAI will not invent attractions.',crowdRisk:'Unknown crowd',source:'Destination geocoding — Open-Meteo'}]}];
    } state.lastPlan.poiCount=state.pois.filter(p=>poiName(p)).length;
    if(CFG.AI_ENDPOINT){ try{ setLoading(true,'YatraAI is re-planning…','Re-checking the evidence and rebuilding the itinerary.'); await enrichWithAI(state.lastPlan,{language:$('#tripLanguage').value,budget:$('#budget').value,notes:$('#notes').value}); }catch(e){state.aiConnected=false; console.warn(e);} }
    renderResults(state.lastPlan); toast('Plan refreshed from live APIs.'); }catch(e){toast('Live refresh failed. Keeping the last verified snapshot.');} finally{setLoading(false);} }
...
$('#heroPlanBtn').addEventListener('click',()=>{ $('#planner').scrollIntoView({behavior:'smooth',block:'start'}); setTimeout(()=>$('#destination').focus(),450); });
